...
2. Entering a room creates canonical room state through `core/game-state.js`: local participant, pointer target, a capped deterministic touch-star pool mapped onto all-sky constellation nodes around the playable space, empty constellation progress, owned shared bot slots, status, and empty peer/pulse/resonance collections.
