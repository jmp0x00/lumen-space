...
- GitLab Pages/GitDocs static hosting from the repository `docs` folder.
...
- `scoreboard-simulation.js` owns the pure completed-room sample state used by the simulator's scoreboard preview, including deterministic reveal attribution, completed progress, opened stars, and selector-derived summary metrics.
...
- `app-ui.js` owns default DOM rendering for the lobby, room chrome, compact objective guidance, room progress stats, full-map completion scoreboard overlay with a Leave action, participant roster, icon actions, and toast, plus a scene-only generator for embedded clients.
...
- `simulator.html`, `physics-sim.css`, and `physics-sim.js` form a separate static inspection app that runs scripted peers against the same pure physics modules, embeds realtime app clients, previews the completed scoreboard surface, and exposes the separate song mode.
- `scoreboard-sim.html` is a standalone iframe-friendly preview that uses the real default room UI renderer, Three.js scene renderer, and `scoreboard-simulation.js` completed-room state so the full-map leaderboard can be visually checked without WebRTC or manual gameplay.
...
- `docs/index.html` is the GitLab Pages/GitDocs docs-root entry point. It preserves query/hash state and redirects visitors into the playable static app under `docs/app`.
...
14. The simulator's scoreboard mode embeds `scoreboard-sim.html`, which creates a deterministic completed room through `scoreboard-simulation.js`, renders the real full-map scene and completion overlay, then posts summary metrics back to the simulator side panel.
...
Scoreboard mode embeds `scoreboard-sim.html`, a standalone preview page backed by `scoreboard-simulation.js`. The preview creates a deterministic completed room with four sample reveal leaders, opens all 767 touch stars, marks all 88 constellations complete, and then runs the same default app UI renderer plus the same Three.js scene renderer used by the playable room. This keeps completion-scoreboard visual checks fast while avoiding a debug-only branch inside the playable app.
...
- Use GitLab Pages/GitDocs-friendly static files instead of a build system.
- Publish the repository `docs` folder through GitLab Pages by copying it into the CI `public` artifact, and keep the playable app under `docs/app` with a docs-root redirect page instead of a build workflow.
